...
class RequestHTTP_1_1(Transport):
    ...
    def __init__(self, cfg, build_session):
        
        self.cfg = cfg
        self._session = build_session
        
        self.url = self._get_url(self.cfg.SERVER.server_url)
    ...
    def _post_bytes_multi_user(self, data : bytes):
        ...
        with ThreadPoolExecutor(max_workers=self.cfg.SERVER.concurrency) as exe:
            futures = [exe.submit(send_once) for _ in range(self.cfg.SERVER.total_requests)]
        return futures
    ...
    def post(self, data):
        ...
        handler = handlers.get(self.cfg.SERVER.content_type)
        r = handler(data)
